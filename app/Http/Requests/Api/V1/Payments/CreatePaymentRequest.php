...
class CreatePaymentRequest extends BaseRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'provider' => 'required|in:stripe,pay-pall',
    ...
    public function makeDTO() : CreatePaymentDTO {
        ...
        return new CreatePaymentDTO($uuid, $title, $currency, $callbackUrl, $sessionCode, $provider, $itemCollection);
    }
    ...
    protected function createPaymentItems(array $itemsData, string $currency): array {
        try {
            return array_map(function($item) use ($currency) {
                return new PaymentItemDTO($item['name'], Money::of($item['value'], $currency));
            }, $itemsData);
        } catch (\Exception $e) {
            throw ValidationException::withMessages([
                'value' => ['Item value validation error'],
            ]);
        }
    }
